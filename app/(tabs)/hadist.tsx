...
function HadithCard({ item, bookName, bookId, router }: { item: Hadith; bookName: string; bookId: string; router: any }) {
    // Extract a "title" from the Indonesian text - usually the first sentence or first few words
    const title = useMemo(() => {
        const firstPeriod = item.id.indexOf('.');
        const snippet = firstPeriod !== -1 && firstPeriod < 60
            ? item.id.substring(0, firstPeriod).trim()
            : item.id.substring(0, 50).trim() + '...';
        return snippet.replace(/^Dari\s+[A-Za-z]+\s+(.*?)\s+berkata:?\s*/i, '$1').trim() || `Hadits ${item.number}`;
    }, [item.id, item.number]);
    ...
    return (
        <View style={styles.card}>
            ...
            <Text style={styles.arabicText} numberOfLines={3}>{item.arab}</Text>
...
export default function HadithScreen() {
        ...
        const timer = setTimeout(() => {
            ...
            for (const part of parts) {
                const bookMatch = books.find(b =>
                    b.id.includes(part) ||
                    b.name.toLowerCase().includes(part)
                );
                if (bookMatch) {
    ...
    const filteredHadiths = useMemo(() => {
        ...
        const keywords = parts.filter(p => {
            const isBook = books.some(b => b.id.includes(p) || b.name.toLowerCase().includes(p));
            const isNum = !isNaN(parseInt(p));
            return !isBook && !isNum;
        }).join(" ");

        return hadiths.filter(h => {
            const matchesNumber = h.number.toString().includes(query) || parts.includes(h.number.toString());
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
                ...
                <Text style={styles.headerSubtitle}>Kumpulan hadits shahih dan terpercaya</Text>
