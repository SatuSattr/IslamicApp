...
function DoaCard({
  ...
  const truncatedArab =
    item.arab.length > 50 ? item.arab.substring(0, 50) + "..." : item.arab;
  const truncatedTerjemah =
    item.terjemah.length > 70
      ? item.terjemah.substring(0, 70) + "..."
      : item.terjemah;

  return (
    <Pressable style={styles.card} onPress={onPress}>
      <View style={styles.cardTopRow}>
        <Text style={styles.doaJudul} numberOfLines={2}>
          {item.judul}
        </Text>
      ...
      <Text style={styles.doaArab} numberOfLines={2}>
        {truncatedArab}
      </Text>
      <Text style={styles.doaTerjemah} numberOfLines={3}>
        {truncatedTerjemah}
      </Text>
  ...
  const fetchDoa = async () => {
    ...
    try {
      ...
      if (require("react-native").Platform.OS === "web") {
        url = `https://all-origins-myslf.vercel.app/raw?apikey=1232&url=${encodeURIComponent(
          url
        )}`;
      }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        <Pressable
          style={[
            styles.filterChip,
            filter === "semua" && styles.filterChipActive,
          ]}
        ...
        <Pressable
          style={[
            styles.filterChip,
            filter === "favorit" && styles.filterChipActive,
          ]}
        ...
        >
          <Text
            style={[
              styles.filterText,
              filter === "favorit" && styles.filterTextActive,
            ]}
          >
