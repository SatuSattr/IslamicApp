...
const PlayPulse = ({ color = "#6F8F72", size = 20 }: { color?: string, size?: number }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 2, height: size, justifyContent: 'center', width: size }}>
            <Animated.View style={{ width: size / 4, backgroundColor: color, height: '60%', transform: [{ scaleY: anim1 }], borderRadius: 1 }} />
            <Animated.View style={{ width: size / 4, backgroundColor: color, height: '100%', transform: [{ scaleY: anim2 }], borderRadius: 1 }} />
            <Animated.View style={{ width: size / 4, backgroundColor: color, height: '70%', transform: [{ scaleY: anim3 }], borderRadius: 1 }} />
        </View>
    );
};
...
export default function SurahDetailScreen() {
    ...
    const triggerPlayPauseAnim = () => {
        Animated.sequence([
            Animated.timing(playPauseScale, { toValue: 0.8, duration: 100, useNativeDriver: true }),
            Animated.spring(playPauseScale, { toValue: 1, friction: 4, tension: 40, useNativeDriver: true }),
        ]).start();
    };
    ...
    const renderAyat = ({ item }: { item: Ayat }) => (
        <View style={[styles.ayatContainer, activeAyat === item.nomorAyat && styles.activeAyatBg]}>
            <View style={styles.ayatHeader}>
                ...
                <View style={styles.ayatActions}>
                    <Pressable
                        ...
                        onPress={() => playAudio(item.audio[selectedQoriId], item.nomorAyat)}
                    >
                        {activeAyat === item.nomorAyat ? (
                            isBuffering ? (
                                <ActivityIndicator size="small" color="#6F8F72" style={{ padding: 4 }} />
                            ) : isPlaying ? (
                                <PlayPulse size={18} />
                            ) : (
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
                <View style={styles.headerLeft}>
                    ...
                    <View>
                        ...
                        <Text style={styles.headerSubtitle}>{surah.arti} • {surah.jumlahAyat} Ayat • {surah.tempatTurun}</Text>
                    </View>
                </View>
            ...
            <FlatList
                ...
                contentContainerStyle={[styles.listContent, { paddingBottom: currentAudioUrl ? 120 : 40 }]}
                ListHeaderComponent={
                    <View style={styles.actionHeader}>
                        ...
                        >
                            ...
                            <Text style={styles.playFullText}>
                                {activeAyat === 0 && isPlaying ? 'Sedang Memutar...' : 'Putar Full Surah'}
                            </Text>
                        </Pressable>

                        <Pressable
                            style={[styles.qoriSettingsBtn, (isPlaying || isBuffering) && styles.disabledBtn]}
                            onPress={() => !(isPlaying || isBuffering) && setIsQoriModalVisible(true)}
                ...
                ListFooterComponent={
                    <View style={styles.footerNav}>
                        {surah.suratSebelumnya && (
                            <Pressable
                                style={styles.navBtn}
                                onPress={() => router.replace({ pathname: '/surah-detail', params: { nomor: surah.suratSebelumnya.nomor } })}
                            >
                        ...
                        {surah.suratSelanjutnya && (
                            <Pressable
                                style={styles.navBtn}
                                onPress={() => router.replace({ pathname: '/surah-detail', params: { nomor: surah.suratSelanjutnya.nomor } })}
                            >
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <View style={styles.modalList}>
                            {QORI_LIST.map((qori) => (
                                <Pressable
                                    key={qori.id}
                                    style={[styles.qoriOption, selectedQoriId === qori.id && styles.qoriOptionActive]}
                                    onPress={() => handleSelectQori(qori.id)}
                                >
                                    <View style={styles.qoriInfo}>
                                        <MaterialIcons
                                            name={selectedQoriId === qori.id ? "radio-button-checked" : "radio-button-unchecked"}
                                            size={20}
                                            color={selectedQoriId === qori.id ? "#6F8F72" : "#999"}
                                        />
                                        <Text style={[styles.qoriName, selectedQoriId === qori.id && styles.qoriNameActive]}>
                                            {qori.name}
                                        </Text>
                                    </View>
            ...
            {currentAudioUrl && (
                <Animated.View style={[styles.audioController, { opacity: fadeAnim, bottom: insets.bottom + 10 }]}>
                    <View style={styles.audioInfo}>
                        ...
                        <View>
                            ...
                            <Text style={styles.audioSurahName}>
                                {surah.namaLatin} ({QORI_LIST.find(q => q.id === selectedQoriId)?.name})
                            </Text>
                        </View>
                    </View>
                    <View style={styles.audioActions}>
                        <Pressable onPress={togglePlayback}>
                            <Animated.View style={{ transform: [{ scale: playPauseScale }] }}>
                                {isBuffering ? (
                                    <ActivityIndicator size="large" color="#6F8F72" style={{ width: 44, height: 44 }} />
                                ) : (
                                    <MaterialIcons name={isPlaying ? "pause-circle-filled" : "play-circle-filled"} size={44} color="#6F8F72" />
                                )}
                            </Animated.View>
                        </Pressable>
